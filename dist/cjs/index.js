...
function createNomoToken(req, nomo_config) {
    ...
    const nonce = require$$5__default["default"].randomBytes(15).toString('hex');
    const timestamp = moment__default["default"]().utc().unix();
    const sign_data = { nomo_auth_addr, nomo_eth_addr, nonce, timestamp, ip_addr };
    return jwt__default["default"].sign(sign_data, nomo_config.nomo_token_secret);
}
function validateNomoToken(req, nomo_config) {
    ...
    if (timestamp > moment__default["default"]().utc().unix())
        throw new NomoApiError(403, 'NOMO_AUTH_TOKEN_FROM_FUTURE');
    if (timestamp + nomo_config.nomo_token_validity < moment__default["default"]().utc().unix())
...
function getNomoTokenData(nomo_token, nomo_config) {
    if (!nomo_token)
        throw new NomoApiError(403, 'NOMO_AUTH_TOKEN_INVALID');
    try {
        return jwt__default["default"].verify(nomo_token, nomo_config.nomo_token_secret);
    }
    catch (error) {
        throw new NomoApiError(403, 'NOMO_AUTH_TOKEN_INVALID');
    }
...
function verifyNomoSignature(nomo_auth_addr, nomo_sig, nomo_token) {
    try {
        return message__default["default"].verify(nomo_token, nomo_auth_addr, nomo_sig, '\x19Eurocoin Signed Message:\n');
    }
    catch (e) {
        return false;
    }
...
function verifyEthSignature(nomo_eth_addr, nomo_eth_sig, nomo_token) {
    try {
        const used_eth_addr = verifyMessage(nomo_token, nomo_eth_sig);
...
exports.getNomoHeaderData = getNomoHeaderData;
exports.nomoMiddleware = nomoMiddleware;
